...
app.controller("shopping-cart-ctrl", function ($scope, $http) {
    ...
    $scope.cart = {
        ...
        prod: [],
        add2(product_id) {
            var qty2 = Number(document.getElementById("sticky-cart-qty").value);
            var item = this.items.find(item => (item.product_id == product_id));
            if (item) {
                item.quantity=item.quantity+qty2;
                this.saveToLocalStorage();
            } else {
                $http.get(`/rest/products/${product_id}`).then(resp => {
                    resp.data.quantity = qty2;
        ...
        add1(product_id) {
            var qty = Number(document.getElementById("qtyy").value);
            var item = this.items.find(item => (item.product_id == product_id));
            if (item) {
    ...
    $scope.order = {
        ...
        // },
        purchase1() {
            var p = document.getElementById("phone_thanhtoanpaypal").value;
            var d = document.getElementById("diachi_thanhtoanpaypal").value;
            var vnf_regex = /((09|03|07|08|05)+([0-9]{8})\b)/g;
            console.log("SDT: " + p);
            console.log("DIA CHI: " + d);
            if(p==null || p == ""){
                alert("Bạn chưa nhập số điện thoại!");
            } else if (vnf_regex.test(p) == false) {
    ...
    $scope.order1 = {
        ...
        // },
        purchase() {
            var p = document.getElementById("phone_thanhtoansau").value;
            var d = document.getElementById("diachi_thanhtoansau").value;
            var vnf_regex = /((09|03|07|08|05)+([0-9]{8})\b)/g;
            console.log("SDT: " + p);
            console.log("DIA CHI: " + d);
            if(p==null || p == ""){
    ...
    $scope.addFavo = function(idindex) {
        $scope.formfavo.product.product_id = idindex;
        var idUser = $scope.formfavo.account.username;
        if(idUser == null || idUser == "") {
            alert("Bạn cần đăng nhập để sử dụng chức năng này");
        } else {
            var favo = angular.copy($scope.formfavo);
            $http.post(`/rest/favorites`, favo).then(resp => {
                $scope.message_from_server = (resp.data);
                alert($scope.message_from_server.text);
            }).catch(error => {
                alert("Đã xảy ra lỗi!.");
                console.log("Error", error);
            })
    ...
    $scope.thanhtoansau = function() {
        var userid = $scope.isUsername;
        if(userid==null || userid == "") {
            alert("Chưa đăng nhập!");
        } else {
            $("#collapse_thanhtoanpaypal").collapse('hide');
            $("#collapse_thanhtoansau").collapse('toggle');
        }
    }

    $scope.thanhtoanpaypal = function() {
        var userid = $scope.isUsername;
        if(userid==null || userid == "") {
            alert("Chưa đăng nhập!");
        } else {
            $("#collapse_thanhtoansau").collapse('hide');
            $("#collapse_thanhtoanpaypal").collapse('toggle');
        }
    }
