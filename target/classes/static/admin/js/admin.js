...
app.controller("ad-ctrl", function ($scope, $http) {
    $scope.mess = [];
    $scope.deleteRunk = function() {
        $http.get("/rest/deleteRunk").then(resp => {
            $scope.mess = (resp.data);
            alert($scope.mess.text);
...
app.controller("authority", function ($scope, $http, $location) {
    ...
    $scope.searchAcc = function () {
        var x = document.getElementById("searchKW").value;
        if (x == '') {
            $scope.initialize();
        } else {
...
app.controller("trademark-ctrl", function ($scope, $http) {
    ...
    $scope.addModal = function () {
        $scope.form = {};
        $("#title").text("Thêm mới thương hiệu");
        document.getElementById('themmoi').style.visibility='visible';
        document.getElementById('capnhat').style.visibility='hidden';
        $('#update-thuonghieu').modal('toggle');
    ...
    $scope.edit = function (item) {
        ...
        document.getElementById('themmoi').style.visibility='hidden';
        document.getElementById('capnhat').style.visibility='visible';
        $('#update-thuonghieu').modal('toggle');
    ...
    $scope.search = function () {
        ...
        if (x == '') {
            ...
            $http.get(`/rest/trademarks/search/${x}`).then(resp => {
                $scope.items = resp.data;
                $scope.pager.first()
            });
        }
    ...
    $scope.delete = function (item) {
        var i = item.trademark_id;
        console.log("test: ", typeof(i));
...
app.controller("category-ctrl", function ($scope, $http) {
    ...
    $scope.search = function () {
        ...
        if (x == '') {
            ...
            $http.get(`/rest/categories/search/${x}`).then(resp => {
                $scope.items = resp.data;
                $scope.pager.first()
            });
        }
