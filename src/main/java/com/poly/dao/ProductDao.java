...
public interface ProductDao extends JpaRepository<Product, Integer> {
	...
	@Query(value = "select * from Products where Unit_price between ?1 and ?2 and Category_id like %?3% and Trademark_id like %?4%", nativeQuery = true)
	...
	@Query(value = "select * from Products where Unit_price between ?1 and ?2 and Category_id like %?3% and Trademark_id like %?4% "
			+ "and  Status like %?5%", nativeQuery = true)
	List<Product> findByAllKeyWordAdmin(@Param("MinPrice") Integer unit_price, @Param("MaxPrice") Integer unit_price1,
			@Param("Category_id") String Category_id, @Param("Trademark_id") String Trademark_id,
			@Param("Status") String Status);
	
	@Query(value = "select Image1 from Products", nativeQuery = true)
	String[] findAllImage1();

	@Query(value = "select Image2 from Products", nativeQuery = true)
	String[] findAllImage2();

	@Query(value = "select Image3 from Products", nativeQuery = true)
	String[] findAllImage3();

	@Query(value = "select Image4 from Products", nativeQuery = true)
	String[] findAllImage4();
